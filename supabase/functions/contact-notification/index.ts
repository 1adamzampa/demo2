...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const industry = (body.industry ?? "Not provided").toString().trim();
    const serviceInterestedIn = (body.serviceInterestedIn ?? "Not provided").toString().trim();
...
<tr><td style="padding:4px 12px 4px 0;font-weight:600">Service Interested In:</td><td>${escapeHtml(serviceInterestedIn)}</td></tr>
...
<tr><td style="padding:4px 12px 4px 0;font-weight:600">Submitted At:</td><td>${escapeHtml(submitted)}</td></tr>
    ...
    return new Response(
      JSON.stringify({ success: true, message: "Thank you. Your request has been received. We'll get back to you shortly.", emailSent }),
